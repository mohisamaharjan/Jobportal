...
const CompaniesList = () => {
  ...
  const handleDelete = (id) => {
    const updatedData = companyData.filter((item) => item._id !== id);
    ...
    <div className="max-w-4xl w-full px-6 mx-auto mt-10 bg-white shadow rounded-lg">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="bg-primary text-white py-2 px-4 rounded hover:bg-primary-blue cursor-pointer"
        >
          Add Company
        </button>
      </div>
          ...
          {companyData.map((company) => (
            <tr key={company._id} className="hover:bg-gray-50">
              <td className="p-3 border-b">
                <img src={company.logo} alt="" className="w-16 h-16 object-cover border" />
              </td>
